...
cpdef (np.ndarray) parse_coin(np.ndarray data, float center_i, float center_j, float coin_radius, float height_scale, extended=True):
	# Get matrix
	...
	cdef np.ndarray repeated
	if extended:
		repeated = np.zeros((height, int(width*1.5)), dtype=np.float32)
...
cpdef np.ndarray parse_top_coin(np.ndarray top, extended=True):
	# Crop the image
	top = top[crop_top:crop_bottom, crop_left:crop_right]
	cdef np.ndarray unwrapped = parse_coin(top, zero_pos_view[0, 0], zero_pos_view[0, 1], top_coin_radius, 1, extended=extended)
	return unwrapped

cpdef np.ndarray parse_bottom_coin(np.ndarray bot, extended=True):
	# Crop the image
	bot = bot[crop_top:crop_bottom, crop_left:crop_right]
	cdef np.ndarray unwrapped = parse_coin(bot, zero_pos_view[1, 0], zero_pos_view[1, 1], bot_coin_radius, bt_ratio, extended=extended)
	return unwrapped

cpdef parse_top_bottom(np.ndarray top, np.ndarray bot, extended=True):
	# Parse the text from the top image
	top_thread = ThreadWithReturnValue(target=parse_top_coin, args=[top, extended])
	top_thread.start()

	# Parse the text from the bottom image
	cdef np.ndarray bot_unwrapped = parse_bottom_coin(bot, extended=extended)
...
# dataset = dataset.reshape(-1, dataset_shape[2]*dataset_shape[3]).T
# dataset -= np.min(dataset, axis=0)
# dataset /= np.max(dataset, axis=0)
# dataset = dataset.T.reshape(dataset_shape)
...
cpdef parse_dataset_element(int i, extended=True):
	top_unwrapped, bot_unwrapped = parse_top_bottom(dataset[i, 0], dataset[i, 1], extended=extended)
